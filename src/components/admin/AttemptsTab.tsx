            ...
            <Card key={att.id} className="group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-border/50 hover:border-primary/50 overflow-hidden flex flex-col rounded-2xl bg-card">
              ...
              <div className="p-3 border-t bg-muted/10 flex flex-col gap-1 mt-auto text-xs text-muted-foreground font-medium">
                <div className="flex items-center gap-2">
                  <User className="w-3.5 h-3.5 text-primary" /> ID Peserta: <span className="font-bold text-foreground">{att.student_id || 'Anonim'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <Calendar className="w-3.5 h-3.5 text-primary" /> Selesai: <span className="font-bold text-foreground">{att.finished_at ? new Date(att.finished_at).toLocaleString('id-ID', { day:'numeric', month:'short', hour:'2-digit', minute:'2-digit' }) : '-'}</span>
                </div>
              </div>
            </Card>
          ))}
