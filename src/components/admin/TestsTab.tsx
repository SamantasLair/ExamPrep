  ...
  return (
    <div className="flex-1 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between mb-6">
        ...
        <Button onClick={onCreateNew} className="bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg hover:shadow-xl transition-all gap-2 h-10 px-6 font-bold rounded-full">
      ...
      {testList.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 text-center border-2 border-dashed rounded-2xl bg-muted/10">
          ...
          <Button onClick={onCreateNew} variant="outline" className="gap-2 rounded-full">
            ...
            <Card key={test.id} className="group hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border-border/50 hover:border-primary/50 overflow-hidden flex flex-col rounded-2xl bg-card">
              ...
              <div className="p-4 border-t bg-muted/5 flex items-center justify-between gap-2 mt-auto">
                <div className="flex gap-2">
                  <Button size="sm" variant="ghost" className="text-primary hover:bg-primary/10 hover:text-primary gap-1.5 font-bold px-2" onClick={() => onAnalytics(test.id)} title="Analisis Butir Soal">
                    <LineChart className="w-4 h-4" />
                  </Button>
                  <Button size="sm" variant="ghost" className="text-blue-600 hover:bg-blue-600/10 hover:text-blue-600 gap-1.5 font-bold px-2" onClick={() => onCopyLink(test.id)} title="Salin Tautan Ujian">
                ...
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="gap-1.5 bg-background hover:bg-muted font-semibold px-3" onClick={() => onEdit(test)}>
                    <Edit2 className="w-3.5 h-3.5" /> Edit
                  </Button>
                  <Button size="sm" variant="destructive" className="gap-1.5 shadow-sm px-3" onClick={() => onDelete(test.id)}>
